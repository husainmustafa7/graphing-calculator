// src/utils/normalizeExpression.js
...
export function normalizeExpression(rawExpr) {
  ...
  // Keep 'e' as math constant
  expr = expr.replace(/\be\b/g, "e");

  // Add * between number and variable (e.g., 2x → 2*x)
  expr = expr.replace(/(\d)([a-zA-Z])/g, "$1*$2");

  // Add * between variable and variable (e.g., ax → a*x)
  expr = expr.replace(/([a-zA-Z])([a-zA-Z])/g, "$1*$2");

  // Add * between number and opening parenthesis (2(x+1) → 2*(x+1))
  ...
  // Wrap shorthand functions: sinx → sin(x)
